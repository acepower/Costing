...
class invoiceModel
{
    ...
    public function processData($dataToProcess)
    {
        ...
        foreach ($this->orderDetails as $key => $value) {
            ...
            else if(array_key_exists($key,$piecesQuantity) && $value!=0)
            {
                $this->orderRepeat[$key]=$value;
            }
            else if($key!="Quantity1" && $isMaterial==false)
            {
    ...
    public function processCost($quantity,$generals, $constants, $materials)
    {
        ...
        $cost=array();
        $cost['gluingCost']= $this->calculateGluingCost($quantity, $generals['GluingMinutesPerPiece'],$constants['GluingCost']);
        ...
        if(array_key_exists('PalletCostDropdown',$generals))
            $cost['transportCost']= $this->calculateTransportCost($quantity,$generals['PiecesPerPallet'],$generals['PalletCostDropdown']);
        else
            $cost['transportCost']= $this->calculateTransportCost($quantity,$generals['PiecesPerPallet'],$generals['PalletCost']);

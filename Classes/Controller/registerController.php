   ...
   public function displayErrors($errorMessage)
   {
    ...
    private function handleRegisterData()
    {
        ...
        if ($this->validator_message=="")
        {
            ...
            {
                if(strcmp($sanitized_credentials['Username'],$unique['Username'])==0 && strcmp($sanitized_credentials['Email'],$unique['Email'])==0)
                    $this->validator_message.="Email and username already used";
                else if (strcmp($sanitized_credentials['Email'],$unique['Email'])==0)
